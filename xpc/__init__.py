...
class XPlaneConnect(object):
    ...
    def __init__(self, xpHost='localhost', xpPort=49009, port=0, timeout=100):
        ...
        if xpPort < 0 or xpPort > 65535:
            raise ValueError(
                "The specified X-Plane port is not a valid port number.")
    ...
    def sendDREFs(self, drefs, values):
        ...
        if len(drefs) != len(values):
            raise ValueError(
                "drefs and values must have the same number of elements.")
        ...
        for i in range(len(drefs)):
            ...
            if value is None:
                raise ValueError(
                    "value must be a scalar or sequence of floats.")
            ...
            if hasattr(value, "__len__"):
                ...
                buffer += struct.pack(fmt.encode(), len(dref),
                                      dref.encode(), len(value), value)
            else:
    ...
    def sendTEXT(self, msg, x=-1, y=-1):
        ...
        buffer = struct.pack(
            b"<4sxiiB" + (str(msgLen) + "s").encode(), b"TEXT", x, y, msgLen, msg.encode())
        self.sendUDP(buffer)
    ...
    def sendWYPT(self, op, points):
        ...
        if len(points) % 3 != 0:
            raise ValueError(
                "Invalid points. Points should be divisible by 3.")
        if len(points) / 3 > 255:
            raise ValueError(
                "Too many points. You can only send 255 points at a time.")

        if op == 3:
            buffer = struct.pack(b"<4sxBB", b"WYPT", 3, 0)
        else:
            buffer = struct.pack(
                ("<4sxBB" + str(len(points)) + "f").encode(), b"WYPT", op, len(points), *points)
        self.sendUDP(buffer)
